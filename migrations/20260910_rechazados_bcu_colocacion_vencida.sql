-- Rechazados BCU: persist colocacion_vencida (MN/ME) on institutions.
-- Apply manually AFTER 20260909_rechazados_bcu_html_source_file_sha256.sql.
-- DO NOT apply automatically from agents until reviewed.
--
-- NULL = absent / unknown / not persisted. No DEFAULT 0.
-- Does NOT change Mi Deuda membership, ops/CDV rules, or snapshot.summary shape in app logic.
-- Extends persist_rejected_bcu_observation INSERT to accept colocacion_vencida_* from p_institutions.

BEGIN;

ALTER TABLE public.rejected_bcu_institutions
  ADD COLUMN IF NOT EXISTS colocacion_vencida_mn numeric NULL,
  ADD COLUMN IF NOT EXISTS colocacion_vencida_me numeric NULL;

COMMENT ON COLUMN public.rejected_bcu_institutions.colocacion_vencida_mn IS
  'MN colocación vencida. null = absent/not persisted/illegible under bcu_v1. Not auto-filled as 0 for historical rows.';

COMMENT ON COLUMN public.rejected_bcu_institutions.colocacion_vencida_me IS
  'ME colocación vencida. null = absent/not persisted/illegible under bcu_v1.';
...
ALTER TABLE public.rejected_bcu_institutions
  ADD CONSTRAINT rejected_bcu_institutions_balances_null_or_nonneg_check CHECK (
    ...
    (creditos_reestructurados_me IS NULL OR creditos_reestructurados_me >= 0) AND
    (colocacion_vencida_mn IS NULL OR colocacion_vencida_mn >= 0) AND
    (colocacion_vencida_me IS NULL OR colocacion_vencida_me >= 0)
  );
...
BEGIN
  ...
  FOR v_i IN 0 .. (v_n - 1) LOOP
    ...
    VALUES (
      ...
      NULLIF(v_inst->>'colocacion_vencida_mn', '')::numeric,
      NULLIF(v_inst->>'colocacion_vencida_me', '')::numeric,
...
COMMENT ON FUNCTION public.persist_rejected_bcu_observation(
  ...
  'Persist BCU observation (snapshot+institutions). Includes colocacion_vencida_mn/me. Optional p_file_sha256. Optional draft_id updates Stage 4 draft. source must be manual|llm_assisted|html_import.';

-- Thin Stage 4 wrapper unchanged in signature; still delegates to persist RPC.
